...
@Suite struct UsageReaderTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "peeku-usage-\(UUID().uuidString)", directoryHint: .isDirectory)
...
@Suite struct StatusLineInstallerTests {
    let root: URL
    let collector: URL
    let paths: PeekuPaths
    let installer: HookInstaller

    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "peeku-statusline-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        paths = PeekuPaths(root: root.appending(path: "App Support/Peeku", directoryHint: .isDirectory))
...
@Suite struct UsageServiceTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "peeku-usage-service-\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    @Test func claudeMovesFromSetupToWaitingToNotSharedToConnected() throws {
        ...
        #expect(claude()?.source == .waitingForStatusLine, "Peeku is the status line, but it hasn't run")
