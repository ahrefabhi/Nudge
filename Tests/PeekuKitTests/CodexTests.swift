...
@Suite struct CodexInstallerTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "peeku-codex-\(UUID().uuidString)", directoryHint: .isDirectory)
...
@Suite struct AgentCompatibilityTests {
    @Test func sessionsSavedBeforeCodexSupportLoadAsClaude() throws {
        ...
        old.phase = .working
        let url = FileManager.default.temporaryDirectory.appending(path: "peeku-old-\(UUID().uuidString).json")
        SessionStore(url: url).save([old])
        // Strip the new field, as an older Peeku would have written it.
    ...
    @Test func historySavedBeforeCodexSupportStillLoads() throws {
        let url = FileManager.default.temporaryDirectory.appending(path: "peeku-history-old-\(UUID().uuidString).json")
        let old = """
