...
@Suite struct HistoryRecorderTests {
    ...
    func session(_ kind: SessionKind, since offset: TimeInterval, id: String = "s1", quote: String? = nil) -> PeekuSession {
        PeekuSession(id: id, project: "payments-api", task: "Upgrading Stripe SDK", host: .iTerm, kind: kind,
    ...
    @Test func anEpisodeThatEndedWhilePeekuWasQuitHasNoDuration() {
    ...
    @Test func roundTripsThroughTheStore() throws {
        let url = FileManager.default.temporaryDirectory.appending(path: "peeku-history-\(UUID().uuidString).json")
