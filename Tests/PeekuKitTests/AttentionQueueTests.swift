...
@Suite struct AttentionQueueTests {
    ...
    func session(_ id: String, _ kind: SessionKind, age: TimeInterval) -> PeekuSession {
        PeekuSession(id: id, project: id, task: "", host: .iTerm, kind: kind, since: now.addingTimeInterval(-age))
    }
