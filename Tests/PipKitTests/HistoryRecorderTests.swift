...
@Suite struct HistoryRecorderTests {
    ...
    func session(_ kind: SessionKind, since offset: TimeInterval, id: String = "s1", quote: String? = nil) -> PipSession {
        PipSession(id: id, project: "payments-api", task: "Upgrading Stripe SDK", host: .iTerm, kind: kind,
    ...
    @Test func anEpisodeThatEndedWhilePipWasQuitHasNoDuration() {
    ...
    @Test func roundTripsThroughTheStore() throws {
        let url = FileManager.default.temporaryDirectory.appending(path: "pip-history-\(UUID().uuidString).json")
