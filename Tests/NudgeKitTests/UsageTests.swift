...
@Suite struct UsageReaderTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "nudge-usage-\(UUID().uuidString)", directoryHint: .isDirectory)
...
@Suite struct StatusLineInstallerTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "nudge-statusline-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
...
@Suite struct UsageServiceTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "nudge-usage-service-\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    @Test func claudeMovesFromSetupToWaitingToNotSharedToConnected() throws {
        ...
        #expect(claude()?.source == .waitingForStatusLine, "Nudge is the status line, but it hasn't run")
