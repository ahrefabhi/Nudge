...
@Suite struct HookInstallerTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "nudge-install-\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    @Test func keepsOtherSettingsAndHooksInPlace() throws {
        ...
        let backups = try FileManager.default.contentsOfDirectory(atPath: installer.settingsURL.deletingLastPathComponent().path)
            .filter { $0.hasPrefix("settings.json.nudge-backup-") }
...
@Suite struct InboxReaderTests {
    @Test func readsRecordsInOrderAndDeletesThem() throws {
        let directory = FileManager.default.temporaryDirectory.appending(path: "nudge-inbox-\(UUID().uuidString)", directoryHint: .isDirectory)
...
@Suite struct PipMigrationTests {
    let root: URL
    let legacy: NudgePaths
    let current: NudgePaths
    let settings: [HookInstaller.Target: URL]
    let collector: URL

    init() throws {
        root = FileManager.default.temporaryDirectory.appending(path: "nudge-migration-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        legacy = NudgePaths(root: root.appending(path: "Pip", directoryHint: .isDirectory), collectorName: "pip-hook")
        current = NudgePaths(root: root.appending(path: "Nudge", directoryHint: .isDirectory))
    ...
    func installer(_ target: HookInstaller.Target, _ paths: NudgePaths) -> HookInstaller {
        HookInstaller(target: target, settingsURL: settings[target], paths: paths)
    }

    @Test func movesPipsHooksStatusLineAndData() throws {
        ...
        let moved = try PipMigration(legacy: legacy, current: current, settings: settings).run(collectorSource: collector)
    ...
    @Test func leavesPipAloneWhenTheCollectorIsMissing() throws {
        try installer(.claude, legacy).install(collectorSource: collector)
        #expect(throws: HookInstaller.InstallError.collectorMissing) {
            try PipMigration(legacy: legacy, current: current, settings: settings).run(collectorSource: nil)
        }
    ...
    @Test func doesNothingWithoutPip() throws {
        let migration = PipMigration(legacy: legacy, current: current, settings: settings)
