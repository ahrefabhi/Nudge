...
/// What Nudge knows about one Claude Code or Codex session, from hook events (and, for Claude
...
/// Session state on disk, so what Nudge learned from hooks survives a restart.
public struct SessionStore: Sendable {
    public let url: URL

    public init(url: URL) { self.url = url }
    public init(paths: NudgePaths = .default) { url = paths.root.appending(path: "sessions.json") }
