...
public enum MockSessions {
    ...
    public static func calm(now: Date = Date()) -> [NudgeSession] {
        [
            ...
            NudgeSession(id: "chrome", project: "chrome-extension", branch: "feat/popup-auth", task: "Adding OAuth to the popup",
                       activity: "Running tests · 42 of 118", host: .terminal, kind: .working, since: now.addingTimeInterval(-240)),
            NudgeSession(id: "auth", project: "auth-service", branch: "main", task: "Refreshing session tokens",
    ...
    public static func sample(now: Date = Date()) -> [NudgeSession] {
        apply(.multiple, to: calm(now: now), now: now, staggered: true)
    }

    /// Simulates a hook event arriving for the matching sample session(s).
    public static func apply(_ event: Event, to sessions: [NudgeSession], now: Date = Date(), staggered: Bool = false) -> [NudgeSession] {
    ...
    public static func answered(_ session: NudgeSession, now: Date = Date()) -> NudgeSession {
        var next = session
        next.kind = .working
        next.quote = nil
        next.choices = []
    ...
    private static func payments(_ kind: SessionKind, since: Date) -> NudgeSession {
        NudgeSession(id: "payments", project: "payments-api", branch: "feat/stripe-v17", task: "Upgrading Stripe SDK to v17",
                   host: .iTerm, location: "Tab 1", kind: kind,
                   quote: kind == .permission ? "npm install stripe@17.2.0" : nil, since: since)
    }

    private static func dashboard(_ kind: SessionKind, since: Date) -> NudgeSession {
        NudgeSession(id: "dash", project: "dashboard-v2", branch: "main", task: "Rebuilding chart tooltips",
    ...
    private static func infra(_ kind: SessionKind, since: Date) -> NudgeSession {
        NudgeSession(id: "infra", agent: .codex, project: "infra-terraform", branch: "chore/tf-1.9", task: "Upgrading to Terraform 1.9",
                   host: .iTerm, location: "Tab 2", kind: kind,
                   quote: kind == .error ? "Error acquiring the state lock" : nil, since: since)
    }

    private static func docs(since: Date) -> NudgeSession {
        NudgeSession(id: "docs", project: "docs-site", branch: "main", task: "Migrated 14 pages to MDX",
    ...
    public static func usageAlert(now: Date = Date()) -> NudgeSession {
