...
public final class PhaseMachine {
    ...
    public private(set) var celebrating: NudgeSession?
    ...
    @ObservationIgnored public var onOpen: ((NudgeSession) -> Void)?
    ...
    /// Called when the Usage tab's "Set Up…" is clicked for an agent Nudge can't read yet.
    ...
    @ObservationIgnored public var isInView: ((NudgeSession) -> Bool)?
    ...
    public var working: [NudgeSession] {
    ...
    public var managerRows: [NudgeSession] {
        queue.filter(\.needsYou) + working + finished + idle
    }

    public var focused: NudgeSession? {
        let items = queue
    ...
    public func update(sessions new: [NudgeSession]) {
        ...
        // Sessions that were already finished when Nudge started are not news.
    ...
    public func update(usageAlerts new: [NudgeSession]) {
        let queuedBefore = Set(queue.map(\.attentionKey))
        usageAlerts = new
        pruneResolved()
    ...
    private func react(queuedBefore: Set<String>, chimes: Bool, finished: NudgeSession?) {
    ...
    private func announce() {
        ...
        case .alert:
            scheduleCollapse()
        case .pill:
            // Bursts join the folded popover instead of dropping Nudge again.
    ...
    private func celebrate(_ session: NudgeSession) {
        celebrating = session
