...
public struct UsageAlerts: Sendable {
    ...
    public mutating func update(_ usage: [AgentUsage], rules: [UsageAlertRule], now: Date = Date()) -> [NudgeSession] {
        var alerts: [NudgeSession] = []
    ...
    static func alert(id: String, agent: Agent, window: UsageWindow, threshold: Int, since: Date, now: Date) -> NudgeSession {
        ...
        return NudgeSession(id: id, agent: agent, project: name, task: used, host: .other, kind: .usage,
