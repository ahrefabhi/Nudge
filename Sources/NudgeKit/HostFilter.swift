...
/// An app outside the built-in four that Nudge has seen a session in, e.g. Warp, or `unidentified`
/// for every session whose app Nudge couldn't tell.
public struct OtherApp: Hashable, Sendable {
    ...
    /// Sessions in an app Nudge couldn't identify, such as under tmux, all hide together.
    ...
    public static func seen(sessions: [NudgeSession], history: [HistoryEntry], hidden: [String: String],
...
public struct HostFilter: Sendable {
    ...
    public func hides(_ session: NudgeSession) -> Bool { hides(host: session.host, bundleID: session.hostBundleID) }
