...
/// Body transform, with offsets as a fraction of Pip's size. Scale and rotation pivot at the bottom center.
struct PipPose {
...
enum PipMotion {
    ...
    static func pose(_ mood: PipMood, at t: TimeInterval) -> PipPose {
        switch mood {
        case .idle: breathe(t, 4.5)
        case .working: breathe(t, 2.2)
        case .question:
            PipPose(y: tiltY.looping(t, duration: 2.8), rotation: tilt.looping(t, duration: 2.8))
        case .permission, .multiple: hop(t, 1.8)
        case .error:
            PipPose(x: shakeX.looping(t, duration: 2.6), rotation: shakeRotation.looping(t, duration: 2.6))
        case .success: t < successDuration ? hop(t, 1.3) : PipPose()
        }
    }

    /// The resting pose when nothing animates. Question keeps its head tilt.
    static func rest(_ mood: PipMood) -> PipPose {
        mood == .question ? PipPose(rotation: -9) : PipPose()
    }

    static func blinks(_ mood: PipMood) -> Bool {
        mood != .idle && mood != .success && mood != .error
    }

    private static func breathe(_ t: TimeInterval, _ duration: TimeInterval) -> PipPose {
        PipPose(scaleX: breathX.looping(t, duration: duration), scaleY: breathY.looping(t, duration: duration))
    }

    private static func hop(_ t: TimeInterval, _ duration: TimeInterval) -> PipPose {
        PipPose(y: hopY.looping(t, duration: duration),
