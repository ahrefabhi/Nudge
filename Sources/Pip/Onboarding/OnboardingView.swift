        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            OnboardingList(rows: model.environments) { environment in
                HStack(spacing: 12) {
                    ...
                    PipToggle(isOn: environment.enabled) { model.toggle(environment.host) }
                }
                .padding(.vertical, 11)
...
private struct RowText: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            Text(title).font(.pip(13, .semibold))
            Text(detail)
                .font(.pip(11.5))
