...
struct IslandView: View {
    ...
    /// The light panel below the notch, with Pip hanging between them.
    private func lightPanel(_ layout: LightPanelLayout) -> some View {
        ZStack(alignment: .top) {
            ...
            PipView(mood: layout.pipMood, size: layout.pipSize, extras: layout.count > 1, count: layout.count, lifted: true)
                .modifier(DropIn(size: layout.pipSize))
                .padding(.top, layout.pipTop)
                .id(layout.pipMood)
        }
    }
...
private struct LightPanel<Content: View>: View {
    ...
    @Environment(\.pipStill) private var still
