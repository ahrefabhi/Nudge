...
enum SessionOpener {
    enum Failure: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case .automationDenied(let app): "Allow Pip to control \(app) in System Settings → Privacy & Security → Automation."
            }
        }
    }

    static func open(_ session: PipSession, observed: ObservedSession?) -> Result<Void, Failure> {
        switch session.host {
        case .iTerm:
    ...
    private static func run(_ source: String, app: String) -> Result<Void, Failure> {
        ...
        if let error {
            // -1743: the user hasn't allowed Pip to send Apple Events to this app.
