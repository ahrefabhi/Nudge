...
/// One Claude Code or Codex hook event, trimmed to what Nudge shows, as the collector writes it to the inbox.
public struct HookRecord: Codable, Sendable, Equatable {
    ...
    /// registry, so this is how Nudge tells a Codex session is still running and finds its terminal.
...
/// Nudge's own data folder. `NUDGE_HOME` overrides it, for tests and development.
public struct NudgePaths: Sendable, Equatable {
    public var root: URL
    /// The collector's file name in `bin`.
    public var collectorName: String

    public init(root: URL, collectorName: String = "nudge-hook") {
        self.root = root
        self.collectorName = collectorName
    }

    public static var `default`: NudgePaths {
        if let custom = ProcessInfo.processInfo.environment["NUDGE_HOME"], !custom.isEmpty {
            return NudgePaths(root: URL(filePath: custom, directoryHint: .isDirectory))
        }
        ...
        return NudgePaths(root: support)
    }

    /// Where the app kept its data when it was called Pip, for moving it over once.
    public static var pip: NudgePaths {
        ...
        return NudgePaths(root: support, collectorName: "pip-hook")
    }
    ...
    /// Touched whenever Claude Code runs Nudge's status line, with or without usage in its input.
