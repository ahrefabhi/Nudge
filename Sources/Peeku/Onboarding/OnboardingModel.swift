...
final class OnboardingModel {
    ...
    /// Whether Peeku is Claude Code's status line, which is how it reads Claude usage.
    ...
    @ObservationIgnored private let sessions: () -> [PeekuSession]
    ...
    init(sessions: @escaping () -> [PeekuSession], otherApps: @escaping () -> [OtherApp] = { [] }, hookSetup: HookSetup?) {
    ...
    static func detail(for app: WatchedApp, sessions: [PeekuSession]) -> String {
        switch app {
        case .host(let host):
        ...
        case .other(let other):
            ...
            if other.isUnidentified { return "Sessions where Peeku can't tell the app" }
    ...
    /// The first time, macOS's own prompt adds Peeku to the list; after that, open the pane.
    ...
    /// For when Peeku shows as allowed in System Settings but isn't: that entry belongs to an
    /// older build. Clears Peeku's entries and asks again.
    func resetPermissions() {
        let alert = NSAlert()
        alert.messageText = "Reset Peeku's permissions?"
        alert.informativeText = """
        If Peeku already looks switched on in System Settings but isn't working, macOS is remembering an older \
        build of Peeku. This removes Peeku's Accessibility and Automation entries, then asks again. \
        ...
        guard Permissions.resetPeekuEntries() else {
    ...
    /// Whether to offer the reset: Peeku asked, but still isn't trusted.
