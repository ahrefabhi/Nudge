    ...
    var body: some View {
        VStack(spacing: 22) {
            ZStack(alignment: .top) {
                ...
                PeekuView(mood: .working, size: 76)
                    .padding(.top, 22)
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            OnboardingList(rows: model.environments) { environment in
                HStack(spacing: 12) {
                    ...
                    PeekuToggle(isOn: environment.enabled) { model.toggle(environment.id) }
                }
                .padding(.vertical, 11)
...
private struct PermissionsStep: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 14) {
                PeekuView(mood: .success, size: 40)
...
private struct RowText: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            Text(title).font(.peeku(13, .semibold))
            Text(detail)
                .font(.peeku(11.5))
...
private struct PeekuToggle: View {
    let isOn: Bool
    let action: () -> Void
