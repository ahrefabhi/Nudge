...
/// Peeku's settings, as a native grouped form that follows the system appearance.
struct SettingsView: View {
    ...
    private var general: some View {
        Section {
            ...
            Toggle("Open Peeku at login", isOn: Binding(
            ...
            if model.loginItem == .needsApproval {
                HStack {
                    Text("Allow Peeku in Login Items to finish turning this on.").foregroundStyle(.secondary)
                    Spacer()
            ...
            if model.loginItem == .unavailable {
                Text("Available when Peeku runs as an app, not from `swift run`.").font(.callout).foregroundStyle(.secondary)
            }
    ...
    private var notifications: some View {
        Section {
            ...
            Toggle(isOn: Binding(get: { model.popUpOnFinish }, set: { model.setPopUpOnFinish($0) })) {
                ...
                Text("Otherwise Peeku gives a 3-second wink in the notch.")
            }
        ...
        Section {
            ...
            if let codex = model.setup.codexHooks {
                LabeledContent {
                    ...
                    Text(codex == .installed
                         ...
                         : codex == .incomplete ? "Some hooks are missing or out of date." : "Lets Peeku see Codex sessions and why they wait.")
                }
            }
            LabeledContent {
            ...
            LabeledContent {
                ...
                Text(model.setup.accessibility
                     ...
                     : "Already switched on in System Settings but not working? That entry is for an older Peeku: Reset clears it and asks again.")
            }
            LabeledContent {
            ...
            Text("Peeku only reads session state. It never types into your terminal or approves anything on your behalf.")
                .font(.footnote)
    ...
    private var uninstall: some View {
        Section {
            LabeledContent {
                ...
                Text("Uninstall Peeku")
                Text("Removes its hooks, history, data and login item, then moves Peeku to the Trash.")
            }
        }
    }
