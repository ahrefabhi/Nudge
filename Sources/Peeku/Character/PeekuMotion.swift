...
/// Body transform, with offsets as a fraction of Peeku's size. Scale and rotation pivot at the bottom center.
struct PeekuPose {
...
enum PeekuMotion {
    ...
    static func pose(_ mood: PeekuMood, at t: TimeInterval) -> PeekuPose {
        switch mood {
        case .idle: breathe(t, 4.5)
        case .working: breathe(t, 2.2)
        case .question:
            PeekuPose(y: tiltY.looping(t, duration: 2.8), rotation: tilt.looping(t, duration: 2.8))
        case .permission, .multiple: hop(t, 1.8)
        case .error:
            PeekuPose(x: shakeX.looping(t, duration: 2.6), rotation: shakeRotation.looping(t, duration: 2.6))
        case .success: t < successDuration ? hop(t, 1.3) : PeekuPose()
        }
    }

    /// The resting pose when nothing animates. Question keeps its head tilt.
    static func rest(_ mood: PeekuMood) -> PeekuPose {
        mood == .question ? PeekuPose(rotation: -9) : PeekuPose()
    }

    static func blinks(_ mood: PeekuMood) -> Bool {
        mood != .idle && mood != .success && mood != .error
    }

    private static func breathe(_ t: TimeInterval, _ duration: TimeInterval) -> PeekuPose {
        PeekuPose(scaleX: breathX.looping(t, duration: duration), scaleY: breathY.looping(t, duration: duration))
    }

    private static func hop(_ t: TimeInterval, _ duration: TimeInterval) -> PeekuPose {
        PeekuPose(y: hopY.looping(t, duration: duration),
