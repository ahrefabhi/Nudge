...
/// Peeku, drawn from shapes and scaled from `size`. Inside the island use `flat`, so only the eyes show.
struct PeekuView: View {
    var mood: PeekuMood
    var size: CGFloat
    var flat = false
    ...
    var extras = true
    var count = 3
    /// Gap drawn around the badge, in the color behind Peeku.
    ...
    @Environment(\.peekuStill) private var peekuStill
    @State private var start = Date()

    var body: some View {
        let animated = !still && !reduceMotion && !peekuStill
        TimelineView(.animation(minimumInterval: 1.0 / 60, paused: !animated)) { context in
            PeekuFigure(peeku: self, t: animated ? context.date.timeIntervalSince(start) : nil)
        }
...
private struct PeekuFigure: View {
    let peeku: PeekuView
    /// Elapsed time, or nil for the still pose.
    let t: TimeInterval?

    private var s: CGFloat { peeku.size }
    private var color: Color { peeku.mood.eyeColor }

    var body: some View {
        let pose = t.map { PeekuMotion.pose(peeku.mood, at: $0) } ?? PeekuMotion.rest(peeku.mood)
        ZStack(alignment: .topLeading) {
            ear(left: true)
            ear(left: false)
            bodyShape
            eyes
            if peeku.extras { extras }
            if peeku.showZ && peeku.mood == .idle { sleepyZs }
        }
    ...
    private var bodyShape: some View {
        rimmed(PeekuBodyShape(), outline: true)
            .frame(width: s, height: 0.82 * s)
            .shadow(color: .black.opacity(peeku.lifted ? 0.3 : 0), radius: 9, y: 8)
            .offset(y: 0.14 * s)
    }

    /// Ink fill, plus the rim highlights unless Peeku is flat.
    ...
    private var eyes: some View {
        ...
        let glance = peeku.mood == .working ? (t.map { PeekuMotion.glance.looping($0, duration: 3.4) } ?? 0) : 0
        let blink = PeekuMotion.blinks(peeku.mood) ? (t.map { PeekuMotion.blink.looping($0, duration: 5.5) } ?? 1) : 1
    ...
    private var extras: some View {
        ...
        case .permission, .multiple:
            let b = max(13, 0.32 * s)
            ZStack {
                Circle().fill(peeku.ring).frame(width: b + 4, height: b + 4)
                Circle().fill(color).frame(width: b, height: b)
                Text(peeku.mood == .multiple ? "\(peeku.count)" : "!")
        ...
        case .question:
            let bob = t.map { PeekuMotion.bob.looping($0, duration: 1.6) } ?? 0
            Text("?")
    ...
    private func sparkle(_ d: CGFloat, delay: TimeInterval) -> some View {
        ...
        return Rectangle()
            ...
            .scaleEffect(PeekuMotion.twinkleScale.looping(local, duration: 1.4, delay: delay))
            .rotationEffect(.degrees(45))
            .opacity(PeekuMotion.twinkle.looping(local, duration: 1.4, delay: delay))
    }
    ...
    private func sleepyZ(fontSize: CGFloat, delay: TimeInterval) -> some View {
        let local = t ?? 1
        let progress = PeekuMotion.zProgress.looping(local, duration: 3, delay: delay)
        ...
        return Text("z")
            ...
            .opacity(visible ? PeekuMotion.zOpacity.looping(local, duration: 3, delay: delay) : 0)
    }
}

/// The multiple-waiting character: Peeku with a count, the rest of the queue dimmed behind.
struct PeekuGroup: View {
    var size: CGFloat
    var count: Int

    var body: some View {
        ZStack(alignment: .topLeading) {
            PeekuView(mood: .question, size: size, extras: false, still: true)
                .scaleEffect(0.68, anchor: .bottomLeading)
                .opacity(0.5)
            PeekuView(mood: .error, size: size, extras: false, still: true)
                .scaleEffect(0.68, anchor: .bottomTrailing)
                .opacity(0.5)
                .offset(x: 0.7 * size)
            PeekuView(mood: .multiple, size: size, count: count)
                .offset(x: 0.35 * size)
        }
