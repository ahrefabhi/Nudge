...
final class Updater {
    ...
    init() {
        // Only a real Peeku.app has a feed URL; `swift run` and snapshots skip Sparkle entirely.
    ...
    /// Shows Sparkle's window. Peeku has no Dock icon, so bring it forward first.
