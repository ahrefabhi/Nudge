...
/// tab or window where Peeku can tell, and they've touched the Mac recently. Anything it can't
/// confirm counts as not in view, so Peeku notifies.
@MainActor
enum ForegroundSession {
    ...
    static func isInView(_ session: PeekuSession, observed: ObservedSession?) -> Bool {
