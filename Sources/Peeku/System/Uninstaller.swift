...
/// Removes everything Peeku added to the Mac, after the user confirms: its Claude Code and Codex hooks,
...
enum Uninstaller {
    static func confirmAndUninstall() {
        ...
        let alert = NSAlert()
        alert.messageText = "Uninstall Peeku?"
        alert.informativeText = """
        Peeku will remove its hooks and status line from \(files) (your other settings and hooks stay), delete its history and \
        saved data, stop opening at login, and move itself to the Trash.

        Backups Peeku made of those files (….peeku-backup-…) are left where they are.
        """
        ...
        do {
            ...
            // Puts back the user's own status line, if Peeku had taken its place.
        ...
        try? manager.removeItem(at: PeekuPaths.default.root)
