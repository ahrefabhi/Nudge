...
struct IslandView: View {
    ...
    private func lightPanel(_ layout: LightPanelLayout) -> some View {
        ZStack(alignment: .top) {
            ...
            PeekuView(mood: layout.peekuMood, size: layout.peekuSize, extras: layout.count > 1, count: layout.count, lifted: true)
                .modifier(DropIn(size: layout.peekuSize))
                .padding(.top, layout.peekuTop)
                .id(layout.peekuMood)
        }
    }
    ...
    private func content(wing: CGFloat, expanded: Bool, bar: CGFloat? = nil) -> some View {
        ...
        ZStack(alignment: .top) {
            switch machine.phase {
            // These have nothing to click, and their animations (Peeku's eyes, the spinner) redraw
