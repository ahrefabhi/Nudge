...
/// Nudge drains the inbox within seconds while it runs. A backlog this large means Nudge was
...
/// Claude Code runs `nudge-hook statusline` as its status line command. It saves the subscription
...
func runStatusLine() {
    ...
    markSeen(argument(after: "--usage").map { URL(filePath: $0).deletingLastPathComponent().appending(path: "claude-statusline-seen") }
             ?? NudgePaths.default.claudeStatusLineSeen)
    if let payload = (try? JSONSerialization.jsonObject(with: input)) as? [String: Any],
       let limits = payload["rate_limits"] as? [String: Any] {
        let file = argument(after: "--usage").map { URL(filePath: $0) } ?? NudgePaths.default.claudeUsage
...
/// minute, which is enough for Nudge's "updated … ago".
...
/// Lets Nudge tell "the status line never ran" from "Claude didn't include usage". Touched at most once a minute.
...
func run() {
    ...
    let inbox = argument(after: "--inbox").map { URL(filePath: $0, directoryHint: .isDirectory) } ?? NudgePaths.default.inbox
