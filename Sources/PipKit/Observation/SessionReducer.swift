...
///
/// Only observed facts change a session's phase. Pip does not guess that a session is stuck.
public struct SessionReducer: Sendable {
    ...
    /// Starts from sessions saved before Pip last quit. The registry check that follows drops
    ...
    /// Reconciles with Claude's registry: adds sessions that started before Pip, notices answers
    ...
    public mutating func reconcile(_ entries: [RegistryEntry], now: Date = Date()) {
        ...
        for entry in entries {
            ...
            guard var session = sessions[entry.sessionId] else {
                // First sight, e.g. a session that started before Pip: take the registry's word.
