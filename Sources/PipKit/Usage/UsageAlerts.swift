...
public struct UsageAlerts: Sendable {
    ...
    public mutating func update(_ usage: [AgentUsage], rules: [UsageAlertRule], now: Date = Date()) -> [PipSession] {
        var alerts: [PipSession] = []
    ...
    static func alert(id: String, agent: Agent, window: UsageWindow, threshold: Int, since: Date, now: Date) -> PipSession {
        ...
        return PipSession(id: id, agent: agent, project: name, task: used, host: .other, kind: .usage,
