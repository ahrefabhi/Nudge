...
final class Updater {
    ...
    init() {
        // Only a real Nudge.app has a feed URL; `swift run` and snapshots skip Sparkle entirely.
    ...
    /// Shows Sparkle's window. Nudge has no Dock icon, so bring it forward first.
