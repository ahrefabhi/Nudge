...
/// The two macOS permissions Nudge uses: Accessibility, to raise the exact window, and
/// Automation, to switch iTerm and Terminal tabs.
enum Permissions {
    static var accessibilityTrusted: Bool { AXIsProcessTrusted() }

    /// Adds Nudge to the Accessibility list and shows the system prompt.
    ...
    /// Clears Nudge's own Accessibility and Automation entries, including stale ones an earlier
    /// build left behind (switched on in System Settings, but no longer matching this Nudge).
    /// Only Nudge's bundle ID is touched. Returns whether both resets succeeded.
    @discardableResult
    static func resetNudgeEntries() -> Bool {
