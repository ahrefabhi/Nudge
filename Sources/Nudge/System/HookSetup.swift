...
/// Installing Nudge's Claude Code and Codex hooks, always with the user's explicit go-ahead: the
...
final class HookSetup {
    ...
    static var bundledCollector: URL? {
        let candidates = [
            Bundle.main.bundleURL.appending(path: "Contents/Helpers/nudge-hook"),
            Bundle.main.executableURL?.deletingLastPathComponent().appending(path: "nudge-hook"),
        ]
        return candidates.compactMap { $0 }.first { FileManager.default.isExecutableFile(atPath: $0.path) }
    }

    /// The agents this Mac has: Claude Code always (it's Nudge's first agent), Codex when installed.
    ...
    func confirmAndInstall(_ target: Target = .claude) {
        ...
        let alert = NSAlert()
        alert.messageText = updating ? "Update \(agent.productName) hooks?" : "Let Nudge watch your \(agent.productName) sessions?"
        var text = """
        Nudge will add \(installer.events.count) hook entries to \(tilde(installer.settingsURL)). Each one runs Nudge's \
        collector, which notes what \(agent.name) is doing so Nudge can show it. It never answers or approves anything.

        Your other settings and hooks stay as they are, and a backup is saved next to the file first. \
        Sessions that are already running may need a restart to report to Nudge.
        """
    ...
    /// Codex skips new hooks until the user trusts them; Nudge can't (and shouldn't) do that for them.
    static let codexTrustStep = "Codex runs new hooks only after you trust them: in Codex, type /hooks and trust Nudge's entries."
    ...
    func confirmAndRemove(_ target: Target = .claude) {
        let installer = HookInstaller(target: target)
        let alert = NSAlert()
        alert.messageText = "Remove Nudge's \(target.agent.productName) hooks?"
        alert.informativeText = target == .claude
            ? "Only Nudge's entries are removed from \(tilde(installer.settingsURL)). Nudge will still list running sessions, but can't say why they're waiting."
            : "Only Nudge's entries are removed from \(tilde(installer.settingsURL)). Nudge will stop seeing Codex sessions."
    ...
    /// Claude Code shares subscription usage only with its status line, so Nudge becomes it.
    ...
    func confirmAndInstallStatusLine() -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = "Show your Claude usage in Nudge?"
        alert.informativeText = """
        Claude Code shares your 5-hour and weekly limits only with its status line, so Nudge will set \
        \(tilde(installer.settingsURL))'s "statusLine" to run Nudge's collector, which saves those numbers for Nudge to show.

        \(hasOwn ? "Your own status line keeps working: Nudge runs it after saving the numbers, and puts it back if you remove this."
                 : "Nudge's status line is blank. With any status line set, Claude Code hides some of its footer hints, like \"esc to interrupt\".")
    ...
    func confirmAndRemoveStatusLine() {
        ...
        alert.informativeText = "Nudge will put back the status line you had before, or remove its own."
    ...
    private func showCodexTrustReminder() {
        ...
        alert.informativeText = Self.codexTrustStep + " Until then Codex skips them, and Nudge won't see Codex sessions."
