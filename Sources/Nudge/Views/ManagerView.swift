...
struct ManagerView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            HStack {
                HStack(spacing: 8) {
                    // The light panel has Nudge hanging above it instead.
                    // Hidden in the light panel (Nudge hangs above it) and when empty (the empty state has its own Nudge).
                    if !palette.isLight && !machine.sessions.isEmpty {
                        NudgeView(mood: headerMood(need), size: 22, flat: true, extras: false)
                    }
                    Text(need.isEmpty ? "All clear" : "\(need.count) need\(need.count == 1 ? "s" : "") you")
                        .font(.nudge(12, .semibold))
    ...
    @ViewBuilder
    private func nowList(_ need: [NudgeSession]) -> some View {
    ...
    private func sessionList(_ need: [NudgeSession]) -> some View {
    ...
    private func open(_ session: NudgeSession) { machine.open(session.id) }

    private func mark(_ symbol: String, _ session: NudgeSession) -> some View {
        Text(symbol).font(.nudge(11)).foregroundStyle(palette.accent(session.kind))
    }

    private func headerMood(_ need: [NudgeSession]) -> NudgeMood {
        switch need.count {
        case 0: .idle
...
private struct NeedsYouRow: View {
    @Environment(\.palette) private var palette
    let session: NudgeSession
    let onOpen: (NudgeSession) -> Void
...
/// The Now tab with no Claude Code sessions: a sleepy Nudge, what to do next, and a button for
/// each app Nudge watches that's installed and turned on.
private struct EmptyNow: View {
    ...
    var body: some View {
        VStack(spacing: 14) {
            // The light panel already has Nudge hanging above it.
            if !palette.isLight {
                NudgeView(mood: .idle, size: 56, showZ: true)
                    .padding(.top, 8)
            }
            VStack(spacing: 6) {
                Text("Nothing running")
                    .font(.nudge(15, .semibold))
                    .tracking(-0.15)
                    .foregroundStyle(palette.primary)
