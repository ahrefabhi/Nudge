...
struct Spinner: View {
    ...
    var body: some View {
        let still = reduceMotion || nudgeStill
...
/// Nudge dropping out of the notch: from −130% and stretched, a squash, a small rebound, rest.
struct DropIn: ViewModifier {
    ...
    func body(content: Content) -> some View {
        let done = finished || reduceMotion || nudgeStill
