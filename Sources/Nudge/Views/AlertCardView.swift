...
struct AlertCardView: View {
    ...
    var body: some View {
        if palette.isLight {
            // In the light panel Nudge hangs above, so there's no wing row and no Nudge column.
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(alignment: .top, spacing: 14) {
                    NudgeView(mood: session.kind.mood, size: 50)
                        .frame(width: 58)
                        .padding(.top, 2)
                    details
                }
            ...
            HStack(spacing: 8) {
                Button(action: onOpen) {
                    HStack(spacing: 8) {
                        ...
                        Text("↵").font(.nudge(11)).opacity(0.45)
                    }
                }
