...
/// Several agents waiting: one Nudge, one queue, most urgent first.
struct MultiAlertView: View {
    @Environment(\.palette) private var palette
    let queue: [NudgeSession]
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 12) {
                if !palette.isLight { NudgeGroup(size: 42, count: queue.count) }
    ...
    private func row(_ session: NudgeSession, highlighted: Bool) -> some View {
