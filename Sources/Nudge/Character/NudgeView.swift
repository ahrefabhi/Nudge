...
/// Nudge, drawn from shapes and scaled from `size`. Inside the island use `flat`, so only the eyes show.
struct NudgeView: View {
    ...
    var extras = true
    var count = 3
    /// Gap drawn around the badge, in the color behind Nudge.
    ...
    @Environment(\.nudgeStill) private var nudgeStill
    @State private var start = Date()

    var body: some View {
        let animated = !still && !reduceMotion && !nudgeStill
        TimelineView(.animation(minimumInterval: 1.0 / 60, paused: !animated)) { context in
            NudgeFigure(nudge: self, t: animated ? context.date.timeIntervalSince(start) : nil)
        }
...
private struct NudgeFigure: View {
    let nudge: NudgeView
    /// Elapsed time, or nil for the still pose.
    let t: TimeInterval?

    private var s: CGFloat { nudge.size }
    private var color: Color { nudge.mood.eyeColor }

    var body: some View {
        let pose = t.map { NudgeMotion.pose(nudge.mood, at: $0) } ?? NudgeMotion.rest(nudge.mood)
        ZStack(alignment: .topLeading) {
            ear(left: true)
            ear(left: false)
            bodyShape
            eyes
            if nudge.extras { extras }
            if nudge.showZ && nudge.mood == .idle { sleepyZs }
        }
    ...
    private var bodyShape: some View {
        rimmed(NudgeBodyShape(), outline: true)
            .frame(width: s, height: 0.82 * s)
            .shadow(color: .black.opacity(nudge.lifted ? 0.3 : 0), radius: 9, y: 8)
            .offset(y: 0.14 * s)
    }

    /// Ink fill, plus the rim highlights unless Nudge is flat.
    ...
    private var eyes: some View {
        ...
        let glance = nudge.mood == .working ? (t.map { NudgeMotion.glance.looping($0, duration: 3.4) } ?? 0) : 0
        let blink = NudgeMotion.blinks(nudge.mood) ? (t.map { NudgeMotion.blink.looping($0, duration: 5.5) } ?? 1) : 1
    ...
    private var extras: some View {
        ...
        case .permission, .multiple:
            let b = max(13, 0.32 * s)
            ZStack {
                Circle().fill(nudge.ring).frame(width: b + 4, height: b + 4)
                Circle().fill(color).frame(width: b, height: b)
                Text(nudge.mood == .multiple ? "\(nudge.count)" : "!")
        ...
        case .question:
            let bob = t.map { NudgeMotion.bob.looping($0, duration: 1.6) } ?? 0
            Text("?")
    ...
    private func sparkle(_ d: CGFloat, delay: TimeInterval) -> some View {
        ...
        return Rectangle()
            ...
            .scaleEffect(NudgeMotion.twinkleScale.looping(local, duration: 1.4, delay: delay))
            .rotationEffect(.degrees(45))
            .opacity(NudgeMotion.twinkle.looping(local, duration: 1.4, delay: delay))
    }
    ...
    private func sleepyZ(fontSize: CGFloat, delay: TimeInterval) -> some View {
        let local = t ?? 1
        let progress = NudgeMotion.zProgress.looping(local, duration: 3, delay: delay)
        ...
        return Text("z")
            ...
            .opacity(visible ? NudgeMotion.zOpacity.looping(local, duration: 3, delay: delay) : 0)
    }
}

/// The multiple-waiting character: Nudge with a count, the rest of the queue dimmed behind.
struct NudgeGroup: View {
    var size: CGFloat
    var count: Int

    var body: some View {
        ZStack(alignment: .topLeading) {
            NudgeView(mood: .question, size: size, extras: false, still: true)
                .scaleEffect(0.68, anchor: .bottomLeading)
                .opacity(0.5)
            NudgeView(mood: .error, size: size, extras: false, still: true)
                .scaleEffect(0.68, anchor: .bottomTrailing)
                .opacity(0.5)
                .offset(x: 0.7 * size)
            NudgeView(mood: .multiple, size: size, count: count)
                .offset(x: 0.35 * size)
        }
