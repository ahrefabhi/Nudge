...
struct IslandMetrics: Equatable {
    ...
    private static func alertHeight(_ session: NudgeSession?) -> CGFloat {
...
/// Light mode: the notch stays black, Nudge hangs from it, and content sits in a light panel below.
struct LightPanelLayout: Equatable {
    var width: CGFloat
    var height: CGFloat
    /// Distance from the top of the screen to the panel.
    var top: CGFloat
    var nudgeSize: CGFloat
    var nudgeTop: CGFloat
    var nudgeMood: NudgeMood
    /// Shown on Nudge's badge when several agents wait.
    var count: Int
    ...
    static func make(for machine: PhaseMachine, notch: NotchGeometry) -> LightPanelLayout? {
        ...
        case .alert:
            ...
            // Nudge hangs lower when it wears a count badge, so the badge clears the notch.
            let nudgeTop = bar - (multiple ? 2 : 8)
            return LightPanelLayout(width: 400, height: design, top: nudgeTop + 46, nudgeSize: 40, nudgeTop: nudgeTop,
                                    nudgeMood: multiple ? .multiple : first.kind.mood, count: queue.count, fitsContent: true)
        case .manager:
            let need = machine.queue.filter(\.needsYou)
            let mood: NudgeMood = need.isEmpty ? .idle : need.count == 1 ? need[0].kind.mood : .multiple
            let nudgeTop = bar - (need.count > 1 ? 1 : 6)
            return LightPanelLayout(width: 440, height: IslandMetrics.managerIsEmpty(machine) ? 330 : 560, top: nudgeTop + 40,
                                    nudgeSize: 34, nudgeTop: nudgeTop,
                                    nudgeMood: mood, count: need.count, fitsContent: false)
        default:
            return nil
        }
    }
