...
/// Moves an install from one of Peeku's old names (Nudge, and Pip before it) over to Peeku, once:
/// its hook handlers and status line, which point at the old collector, then its data folder
/// (history, saved sessions, usage). The old folder is removed only after every handler has moved,
/// so a failure leaves the old handlers working and the next launch tries again.
public struct LegacyMigration: Sendable {
    public let legacy: PeekuPaths
    public let current: PeekuPaths
    /// Each agent's settings file; the agent's usual file when left out.
    public let settings: [HookInstaller.Target: URL]

    public init(legacy: PeekuPaths, current: PeekuPaths = .default, settings: [HookInstaller.Target: URL] = [:]) {
        self.legacy = legacy
    ...
    public func run(collectorSource: URL?) throws -> [HookInstaller.Target] {
        ...
        for target in HookInstaller.Target.allCases {
            ...
            if hadStatusLine {
                // Puts back the user's own status line, which Peeku's then carries again.
    ...
    /// Moves everything but the old collector into Peeku's folder, keeping anything Peeku already has.
    ...
    /// Folders like the inbox may already exist on both sides; files already in Peeku's win.
