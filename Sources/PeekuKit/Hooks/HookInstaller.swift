...
/// Adds and removes Peeku's hook handlers in Claude Code's settings or Codex's hooks file.
///
/// It only ever touches handlers whose command is Peeku's collector, keeps every other key in
...
public struct HookInstaller: Sendable {
    ...
    public enum Target: String, Sendable, CaseIterable {
        ...
        /// The events Peeku reads. Each gets one handler that runs the collector.
        ...
        /// Claude's Notification fires for several reasons; Peeku only needs these two.
    ...
    public enum Status: Equatable, Sendable {
        case notInstalled
        case installed
        /// Some events have no Peeku handler, or the collector binary is missing or out of date.
        case incomplete
    }
    ...
    public enum InstallError: Error, Equatable, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .settingsChanged: "The settings file changed while Peeku was editing it. Try again."
            case .collectorMissing: "Peeku's hook collector wasn't found in the app."
            }
        }
    }
    ...
    public let paths: PeekuPaths

    public init(target: Target = .claude, settingsURL: URL? = nil, paths: PeekuPaths = .default) {
        self.target = target
    ...
    public func status(bundledCollector: URL?) -> Status {
        ...
        let installed = events.filter { hasPeekuHandler(in: settings["hooks"]?[$0]) }
    ...
    public func install(collectorSource: URL) throws -> Int {
        ...
        for event in events {
            ...
            if hasPeekuHandler(in: groups) { continue }
    ...
    /// Removes Peeku's handlers, and any groups or events left empty by that. Returns how many were removed.
    @discardableResult
    public func uninstall() throws -> Int {
        ...
        for member in events {
            ...
            for group in groups {
                ...
                let others = handlers.filter { !isPeekuHandler($0) }
    ...
    // Claude Code passes subscription usage only to its status line command, so Peeku can become
    // that command. A status line the user already had keeps running: its command is carried,
    // base64-encoded, after `--then`, and put back when Peeku's is removed.

    public func statusLineStatus(bundledCollector: URL?) -> Status {
        guard target == .claude, let settings = try? readSettings().value,
              let command = settings["statusLine"]?["command"]?.stringValue, isPeekuStatusLine(command) else { return .notInstalled }
    ...
    public func installStatusLine(collectorSource: URL) throws -> Bool {
        ...
        try installCollector(from: collectorSource)
        if let previous, isPeekuStatusLine(previous) { return false }
    ...
    /// Puts back the status line the user had before, or removes the key. Returns false when Peeku's wasn't there.
    @discardableResult
    public func uninstallStatusLine() throws -> Bool {
        let original = try readSettings()
        var settings = original.value
        guard var line = settings["statusLine"], let command = line["command"]?.stringValue, isPeekuStatusLine(command) else { return false }
    ...
    func isPeekuStatusLine(_ command: String) -> Bool {
    ...
    func isPeekuHandler(_ handler: OrderedJSON) -> Bool {
    ...
    func hasPeekuHandler(in groups: OrderedJSON?) -> Bool {
        groups?.arrayValue?.contains { group in group["hooks"]?.arrayValue?.contains(where: isPeekuHandler) == true } == true
    }
    ...
    private func write(_ settings: OrderedJSON, replacing original: Snapshot) throws {
        ...
        if let data = original.data {
            ...
            let backup = directory.appending(path: "\(settingsURL.lastPathComponent).peeku-backup-\(stamp)")
            manager.createFile(atPath: backup.path, contents: data, attributes: [.posixPermissions: 0o600])
        }

        let temporary = directory.appending(path: ".\(settingsURL.lastPathComponent).peeku-\(UUID().uuidString).tmp")
    ...
    private func installCollector(from source: URL) throws {
        ...
        let staged = destination.deletingLastPathComponent().appending(path: ".peeku-hook-\(UUID().uuidString).tmp")
    ...
    /// Records what Peeku added, for support and for a future clean uninstall.
