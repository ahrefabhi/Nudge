...
public final class UsageService {
    ...
    private let paths: PeekuPaths
    private let claudeSettings: URL
    private let codexSessions: URL
    private let codexInstalled: @Sendable () -> Bool
    private let io = DispatchQueue(label: "app.peeku.usage", qos: .utility)
    private let live = DispatchQueue(label: "app.peeku.usage.live", qos: .utility)
    private var timer: Timer?
    private let askAgents: Bool
    /// When Peeku last asked the agents, and Codex's own answer.
    ...
    /// Opening the Usage tab asks again, unless Peeku just did.
    static let onDemandInterval: TimeInterval = 60

    public init(paths: PeekuPaths = .default, claudeSettings: URL = ClaudePaths.settings, codexSessions: URL = CodexPaths.sessions,
    ...
    nonisolated static func read(paths: PeekuPaths, claudeSettings: URL, codexSessions: URL, codexInstalled: Bool,
