...
/// running, and Peeku never touches Codex's credentials. The session logs stay the fallback.
public enum CodexLiveUsage {
    public static func read(codex: URL? = AgentCLI.locate("codex"), now: Date = Date(), timeout: TimeInterval = 20) -> UsageReport? {
        ...
        let requests = [
            #"{"id":1,"method":"initialize","params":{"clientInfo":{"name":"peeku","version":"1"}}}"#,
            #"{"method":"initialized"}"#,
...
/// running and no model call. Peeku never touches Claude's credentials.
public enum ClaudeLiveUsage {
    ...
    /// Loads no settings, so none of the user's hooks (Peeku's included), status line or MCP
