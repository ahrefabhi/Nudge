...
public struct HistoryEntry: Codable, Identifiable, Sendable, Equatable {
    public enum Kind: String, Codable, Sendable, CaseIterable {
        ...
        init?(_ kind: SessionKind) {
            ...
            case .idle, .working, .usage, .command, .commandInput: return nil
            }
        }

        /// Needed you: Peeku asked for your attention.
    ...
    /// When a waiting episode ended. Nil while it's still open, or when Peeku wasn't running to see it end.
    ...
    /// What the row shows as the app, like `PeekuSession.hostName`.
