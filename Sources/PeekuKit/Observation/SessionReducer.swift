...
///
/// Only observed facts change a session's phase. Peeku does not guess that a session is stuck.
public struct SessionReducer: Sendable {
    ...
    /// Codex has no registry. A Codex session whose process Peeku couldn't find is kept this long
    ...
    /// Starts from sessions saved before Peeku last quit. The registry check that follows drops
    ...
    public mutating func apply(_ record: HookRecord) {
        ...
        case "Notification":
            // Both usually follow a PermissionRequest or AskUserQuestion that Peeku already shows
    ...
    /// Reconciles with Claude's registry: adds sessions that started before Peeku, notices answers
    ...
    public mutating func reconcile(_ entries: [RegistryEntry], now: Date = Date()) {
        ...
        for entry in entries {
            ...
            guard var session = sessions[entry.sessionId] else {
                // First sight, e.g. a session that started before Peeku: take the registry's word.
