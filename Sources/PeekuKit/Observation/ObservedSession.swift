...
/// What Peeku knows about one Claude Code or Codex session, from hook events (and, for Claude
...
/// Session state on disk, so what Peeku learned from hooks survives a restart.
public struct SessionStore: Sendable {
    public let url: URL

    public init(url: URL) { self.url = url }
    public init(paths: PeekuPaths = .default) { url = paths.root.appending(path: "sessions.json") }
