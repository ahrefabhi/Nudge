...
/// Watches Peeku's hook inbox and Claude's session registry, and publishes `PeekuSession`s.
/// File reads happen on a background queue; state lives on the main actor.
@MainActor
public final class ObservationService {
    public var onChange: (([PeekuSession]) -> Void)?
    ...
    private let io = DispatchQueue(label: "app.peeku.observation", qos: .utility)
    ...
    public init(paths: PeekuPaths = .default, registryDirectory: URL = ClaudePaths.sessions) {
        inbox = InboxReader(directory: paths.inbox)
        self.registryDirectory = registryDirectory
        store = SessionStore(paths: paths)
        // What hooks said before Peeku last quit, e.g. a question's choices.
